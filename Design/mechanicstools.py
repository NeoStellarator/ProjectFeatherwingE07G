...
def macaulay_deflection(forces, moments, E, I, L): # maybe remove this one
    # determine the tip deflection on a clamped beam from the forces and moments
...
def superposition_deflection(forces, E, I, L): # maybe remove this one
    # determine the tip deflection on the clamped beam (with only point forces) 
    # using superposition and std. sol.
    deflect = 0
    
    
    for z, f in forces:
        # first term is the deflection by the force itself
        # second term is the deflection due to curvature at the point force
        deflect += f*z**3/3+f*(L-z)*z**2/2 
        
    deflect /= (E*I)
    
    return deflect

def bending_dist_all(M_tab, I, consistent=True):
    ...
    dy = 0.0001 # [m] precision
    
    M = np.array(M_tab)
    size_M = np.shape(M)[0]
    M = np.reshape(M, (1, size_M))

    y = np.arange(-y_max, y_max+dy, dy)
    size_y = np.shape(y)[0]
    y = np.reshape(y, (size_y, 1))

    stress_matrix = np.matmul(y, M)/I
    
    y = np.reshape(y, (size_y))
    
    return y, stress_matrix

def shear_dist_all(V_tab, I):
    ...
    # Define thickness as functions of y
    t = np.zeros((size_y))
    t = np.where(np.abs(y)>=55e-3, 3.8e-3, 0.8e-3)
    
    # Initialize Q
    Q = np.zeros((size_y))
    Q_val = np.array(4496.356e-9) # value calculated manually for y > 73.5 mm
    ...
    R = Q/(I*t)
    R = np.reshape(R, (size_y, 1)) 
    
    V = np.array(V_tab)
    size_V = np.shape(V)[0]
    V = np.reshape(V, (1, size_V))
    
    stress_matrix = np.matmul(R, V)
    
    return y, stress_matrix

def plot_dist(typ, y, sig, showplot=False, saveplot=False, filename="Stress_Distribution.svg"):
    '''
    Generate a plot for a desired distribution.

    Parameters
    ----------
    typ : STR
        Type of distribution. Either a bending stress distribution "M", or shear
        stress distribution "V"
    y : LIST
        "x-axis" of the plot. Labelled y to be consistent with coordinate system.
    sig : LIST
        "y-axis" of the plot. Labelled sig to represent a (normal or shear) stress.
    showplot : BOOL, optional
        True if the stress distribution diagrams want to be shown.
        The default is False.
    saveplot : BOOL, optional
        True if the internal force (and moment) diagrams want to be saved.
        The default is False.
    filename : STRING, optional
        If the saveplot option is selected, the filename should be introduced.
        The default is "Stress_Distribution.svg".
    ...
    plt.title(name + " Stress Distribution")
    plt.style.use("seaborn-v0_8")
    plt.grid(True)
    plt.xlabel(r'$y$' + " [m]")
    plt.ylabel(symbol + " [Pa]")
    plt.vlines(0, 0,np.max(np.abs(sig)), linestyle='--', lw=0.8)
    plt.hlines(0, y[0],y[-1], linestyle='--', lw=0.8)
    plt.plot(y, sig, color=color)
